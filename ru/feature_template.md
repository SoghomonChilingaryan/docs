...
Логирование (правило ответственности):

- Ошибки внешних вызовов логирует `RepositoryImpl`/`DataSource` (тот, кто делает внешний вызов).
- BLoC не дублирует логирование внешних ошибок: он лишь обрабатывает результат/ошибку и обновляет `state`.
